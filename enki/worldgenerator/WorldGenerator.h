...
namespace Enki
{
	...
	public:
		/*!
			...
			\param seed the seed the randomizer should use. This is an optional
			parameter.
			\return a new WorldGenerator object.
		*/
		WorldGenerator(const long long int &seed = -1);
		...
		*/
		WorldGenerator(const int &width, const int &height, const long long int &seed = -1);
		...
		*/
		WorldGenerator(const int &radius, const long long int &seed = -1);
		~WorldGenerator();
		...
		*/
		...
		/*!
			\brief This function simulates an update of the current working world.
			It can be used to simulate what a world would be at different time.
		*/
		void genStep();
